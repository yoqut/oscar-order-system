...
Text routing (ENTER_CANCEL_REASON, ENTER_COMMENT) is handled by the unified
bot_app/handlers/text_router.py catch-all; expose handle_client_text() for it.
"""
import logging
from telebot import types

from ..core.loader import bot
from ..core.use_states import ClientStates
from ..utils.state_manager import StateManager
from ..utils.helpers import notify_user, notify_admins
from ..keyboards.client_kb import rating_keyboard, skip_comment_keyboard
...
async def client_cb_reject_service(call: types.CallbackQuery):
    ...
    await bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id)
    await bot.send_message(call.message.chat.id, "❌ Rad etildi. Iltimos, sababini yozing:")
    await bot.answer_callback_query(call.id)
...
async def client_cb_skip_comment(call: types.CallbackQuery):
    ...
    if str(state) != str(ClientStates.ENTER_COMMENT):
        await bot.answer_callback_query(call.id)
        return
...
# ── Business logic helpers (called from text_router.py) ──────────────────────

async def handle_client_text(message: types.Message, state: str, data: dict) -> bool:
    """
    Dispatch client text input based on current StateManager state.
    Returns True if handled. Called from bot_app/handlers/text_router.py.
    """
    text = message.text.strip()

    if str(state) == str(ClientStates.ENTER_CANCEL_REASON):
        order_id = data.get('order_id')
        is_rejection = data.get('is_rejection', False)

        try:
            order = await Order.objects.select_related(
                'agronomist', 'sales_manager'
            ).aget(pk=order_id)
        except Order.DoesNotExist:
        ...
        notify_text = (
            ...
            f"Order #{order_id}\nMijoz: {client.full_name if client else '—'}\nSabab: {text}"
        )
        await notify_user(bot, order.agronomist.telegram_id, notify_text)
        await notify_user(bot, order.sales_manager.telegram_id, notify_text)
...
async def _save_feedback(telegram_id: int, message, data: dict,
